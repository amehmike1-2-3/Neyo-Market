...
module.exports = async function handler(req, res) {
  ...
  if (action === 'webhook' && req.method === 'POST') {
    /* Always respond 200 immediately — Paystack retries if we don't reply fast */
    ...
    try {
      ...
      if (payload.event !== 'charge.success' || !payload.data) return;

      const txData   = payload.data;
      const reference = txData.reference;
      const amount    = txData.amount / 100; /* kobo → naira */
      const metadata  = txData.metadata || {};

      /* Extract orderId from metadata — set in processPaystack() as trackId */
      const customFields = metadata.custom_fields || [];
      const trackingField = customFields.find(function(f) {
        return f.variable_name === 'tracking' || f.display_name === 'Tracking ID';
      });
      const orderId = metadata.orderId || (trackingField && trackingField.value) || null;

      if (!orderId || !reference) {
        console.warn('[webhook] missing orderId or reference — cannot save order.');
        return;
      }

      console.log('[webhook] charge.success —', orderId, 'ref:', reference, '₦' + amount);

      /* ── Idempotency: skip if order already saved ── */
      const existing = await sql`
        SELECT id, status FROM orders WHERE id = ${String(orderId)} LIMIT 1
      `;
      if (existing.length && ['paid','escrow_held','completed'].includes(existing[0].status)) {
        console.log('[webhook] order already exists, skipping:', orderId);
        return;
      }

      /* ── Extract customer info from metadata ── */
      const customerName  = (customFields.find(function(f){ return f.variable_name === 'customer'; }) || {}).value || 'Customer';
      const customerWA    = (customFields.find(function(f){ return f.variable_name === 'whatsapp'; }) || {}).value || '';
      const customerAddr  = (customFields.find(function(f){ return f.variable_name === 'address';  }) || {}).value || '';
      const customerEmail = txData.customer && txData.customer.email ? txData.customer.email : '';
      ...
      /* ── Look up buyer by email to get userId ── */
      let userId = '';
      if (customerEmail) {
        try {
          const uRows = await sql`SELECT id FROM users WHERE LOWER(email) = LOWER(${customerEmail}) LIMIT 1`;
          if (uRows.length) userId = String(uRows[0].id);
        } catch(e) {}
      }

      /* ── We don't have cart items from webhook — save a minimal order record ──
         The frontend polling (verify) handles full item details if the tab is open.
         Webhook is the safety net for when the browser is completely closed. ── */
      const deliveryCode = generateDVC(String(orderId));
      const orderMode    = customerAddr && customerAddr !== 'Digital Delivery' ? 'physical' : 'digital';
      const isPhysical   = orderMode === 'physical';

      /* Default to free tier split — we don't have seller info from webhook alone */
      const split       = computeSplit(amount, isPhysical, false, 'free');
      const orderStatus = isPhysical ? 'escrow_held' : 'paid';

      await sql`
        INSERT INTO orders (
          id, user_id, customer, items, total, amount,
          platform_fee, seller_payout, affiliate_fee,
          status, collected, mode, ref,
          shipping, delivery_code, date, created_at
        ) VALUES (
          ${String(orderId)},
          ${String(userId)},
          ${JSON.stringify(customer)},
          ...
          ${orderMode},
          ${reference},
          ${isPhysical ? JSON.stringify({ address: customerAddr, whatsapp: customerWA }) : null},
          ${deliveryCode},
          ${new Date().toLocaleDateString()},
          NOW()
        )
      ...
      console.log('[webhook] order saved successfully —', orderId, 'status:', orderStatus);

    } catch(err) {
      /* Never throw — Paystack already got our 200 */
  ...
  if (action === 'download' && req.method === 'GET') {
    ...
    try {
      ...
      if (new Date() > new Date(rec.expires_at)) {
        return res.status(410).send(dlErrorPage('⏰ Link Expired', 'This link expired 48 hours after purchase. Contact support@neyomarket.com.ng with Order ID: <strong>' + rec.order_id + '</strong>'));
      }

      if (parseInt(rec.used_count || 0) >= 10) {
        return res.status(429).send(dlErrorPage('Download Limit Reached', 'This link has been used too many times. Contact support@neyomarket.com.ng with Order ID: <strong>' + rec.order_id + '</strong>'));
      }
      ...
      if (!fileUrl || !fileUrl.startsWith('http')) {
        return res.status(500).send(dlErrorPage('File Not Available', 'Contact support@neyomarket.com.ng with Order ID: <strong>' + rec.order_id + '</strong>'));
      }
      ...
      return res.status(500).send(dlErrorPage('Server Error', 'Something went wrong. Contact support@neyomarket.com.ng'));
    }
  }

  /* ══════════════════════════════════════════════════
     WITHDRAWAL — check seller balance before approval
  ══════════════════════════════════════════════════ */
  if (action === 'check-balance' && req.method === 'GET') {
    const userId = req.query.userId || '';
    if (!userId) return jsonErr(res, 400, 'userId required.');
    const rows = await sql`SELECT seller_balance FROM users WHERE id::text = ${String(userId)} LIMIT 1`;
    if (!rows.length) return jsonErr(res, 404, 'User not found.');
    const balance = parseFloat(rows[0].seller_balance || 0);
    return res.status(200).json({ ok: true, balance });
  }

  /* ══════════════════════════════════════════════════
     WITHDRAWAL — update withdrawal request status
  ══════════════════════════════════════════════════ */
  if (action === 'update-withdrawal-status' && req.method === 'POST') {
    ...
    await sql`
      ...
      WHERE id::text = ${String(withdrawalId)}
    `;
    /* Deduct seller balance when marking completed */
    if (status === 'completed') {
      const wRows = await sql`SELECT user_id, amount FROM withdrawals WHERE id::text = ${String(withdrawalId)} LIMIT 1`;
      if (wRows.length) {
        const { user_id, amount } = wRows[0];
        const amt = parseFloat(amount || 0);
        await sql`
          UPDATE users
          SET seller_balance = GREATEST(0, COALESCE(seller_balance, 0) - ${amt})
          WHERE id::text = ${String(user_id)}
        `;
        await sql`
          UPDATE wallets
          SET balance = GREATEST(0, COALESCE(balance, 0) - ${amt}),
              updated_at = NOW()
          WHERE user_id::text = ${String(user_id)}
        `;
      }
    }
    return res.status(200).json({ ok: true });
  ...
  /* ══════════════════════════════════════════════════
     WITHDRAWAL — approve payout via Paystack transfer
  ══════════════════════════════════════════════════ */
  if (action === 'approve-payout' && req.method === 'POST') {
    const { withdrawalId, userId, amount, netAmount } = req.body || {};
    if (!withdrawalId || !userId || !amount) return jsonErr(res, 400, 'withdrawalId, userId and amount required.');

    /* Fetch seller bank details */
    const uRows = await sql`SELECT name, bank_code, account_number FROM users WHERE id::text = ${String(userId)} LIMIT 1`;
    if (!uRows.length) return jsonErr(res, 404, 'User not found.');
    const { name, bank_code, account_number } = uRows[0];

    if (!bank_code || !account_number) {
      return res.status(200).json({ ok: false, error: 'Seller has no bank details on file.' });
    }

    const net = parseFloat(netAmount || amount);
    ...
    try {
      ...
      if (!recipData.status) return res.status(200).json({ ok: false, error: 'Recipient creation failed: ' + recipData.message });
      ...
      const transferRes = await fetch('https://api.paystack.co/transfer', {
        ...
        body: JSON.stringify({
          ...
          reason:    'NeyoMarket seller withdrawal #' + withdrawalId
        })
      });
      const transferData = await transferRes.json();
      if (!transferData.status) return res.status(200).json({ ok: false, error: 'Transfer failed: ' + transferData.message });

      return res.status(200).json({ ok: true, reference: transferData.data.reference });
  ...
  /* ══════════════════════════════════════════════════
     WITHDRAWAL — refund balance if Paystack fails
  ══════════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════════════════════════
     POST ?action=verify&ref=REFERENCE&orderId=NYO-XXXX
     Called by the onClose polling in index.html when the user leaves
     the browser to pay via their bank app and the popup closes before
     the callback fires. Checks Paystack, saves order if paid.
  ══════════════════════════════════════════════════════════════════ */
  if (action === 'verify' && req.method === 'POST') {
    const ref     = req.query.ref     || (req.body && req.body.reference) || '';
    const orderId = req.query.orderId || (req.body && req.body.orderId)   || '';

    if (!ref) return jsonErr(res, 400, 'ref is required.');

    try {
      /* 1. Idempotency — order already saved, just return success */
      if (orderId) {
        const existing = await sql`
          SELECT id, status FROM orders WHERE id = ${String(orderId)} LIMIT 1
        `;
        if (existing.length && ['paid','escrow_held','completed'].includes(existing[0].status)) {
          return res.status(200).json({ ok: true, paid: true, alreadySaved: true, status: existing[0].status });
        }
      }

      /* 2. Ask Paystack if reference was actually paid */
      const txn = await verifyPaystackPayment(ref);

      if (!txn) {
        /* Paystack unreachable or no data — tell frontend to keep polling */
        return res.status(200).json({ ok: false, pending: true, message: 'Could not reach Paystack yet.' });
      }

      if (txn.status === 'pending') {
        return res.status(200).json({ ok: false, pending: true, message: 'Payment still processing.' });
      }

      if (txn.status !== 'success') {
        return res.status(200).json({ ok: false, paid: false, message: 'Payment was not completed.' });
      }

      /* 3. Payment confirmed — save the order using same logic as ?action=confirm */
      const body      = req.body || {};
      const userId    = body.userId       || '';
      const sellerUserId = body.sellerUserId || '';
      const items     = body.items        || [];
      const total     = txn.amount / 100; /* use Paystack-verified amount */
      const customer  = safeJson(body.customer, {});
      const mode      = body.mode         || 'standard';
      const affCode   = body.affCode      || null;
      const shipping  = safeJson(body.shipping, null);

      const itemList    = Array.isArray(items) ? items : safeJson(items, []);
      const hasPhysical = itemList.some(function(i) { return i.type === 'physical'; });
      const isAllDigital = itemList.length > 0 && itemList.every(function(i) {
        return i.type === 'digital' || i.type === 'course';
      });
      ...
      const split        = computeSplit(total, hasPhysical, hasValidAff, sellerTier);
      const orderStatus  = isAllDigital ? 'paid' : 'escrow_held';
      const finalOrderId = orderId || ref;
      const deliveryCode = generateDVC(String(finalOrderId));
      const cleanAff     = hasValidAff ? rawAff : null;
      const resolvedSellerIdInt = resolvedSellerId ? parseInt(resolvedSellerId) : null;
      ...
      /* Credit seller immediately for digital orders */
      if (isAllDigital && resolvedSellerId && split.sellerPayout > 0) {
        await sql`
          UPDATE users SET seller_balance = COALESCE(seller_balance, 0) + ${split.sellerPayout}
          WHERE id = ${resolvedSellerId}
      ...
      /* Record affiliate commission as pending */
      if (hasValidAff && split.affiliateFee > 0) {
        try {
          const affRows = await sql`SELECT id FROM users WHERE aff_code = ${rawAff} LIMIT 1`;
          if (affRows.length) {
            const affUserId = String(affRows[0].id);
            await sql`
              INSERT INTO affiliate_commissions (aff_user_id, aff_code, order_id, order_amount, commission, status, created_at)
              VALUES (${affUserId}, ${rawAff}, ${String(finalOrderId)}, ${total}, ${split.affiliateFee}, ${'pending'}, NOW())
              ON CONFLICT (order_id) DO NOTHING
            `;
          }
        } catch(e) {}
      ...
      console.log('[payment/verify] order saved via polling —', finalOrderId, '₦' + total);
      ...
      /* Don't block the poll — return pending so frontend retries */
      return res.status(200).json({ ok: false, pending: true, message: 'Verify error: ' + err.message });
    }
  }

  /* ══════════════════════════════════════════════════════════════════
     POST ?action=save-cart
     Called right before Paystack opens. Saves cart snapshot so webhook
     can restore full item details if buyer closes browser after paying.
  ══════════════════════════════════════════════════════════════════ */
  if (action === 'save-cart' && req.method === 'POST') {
    try {
      const { orderId, userId, items, total, customer, mode, sellerUserId } = req.body || {};
      if (!orderId || !items) return jsonErr(res, 400, 'orderId and items required.');
      ...
      await sql`
        INSERT INTO pending_carts (order_id, user_id, seller_id, items, total, customer, mode)
        VALUES (
          ${String(orderId)},
          ${String(userId || '')},
          ${String(sellerUserId || '')},
      ...
      /* Non-fatal — don't block the buyer */
      console.warn('[payment/save-cart] non-fatal:', err.message);
      return res.status(200).json({ ok: true });
    }
  }
