...
async function bustGlobalCache(productId, sellerId, cat, type) {
  try {
    ...
    if (productId) await r.del('products:id:' + productId);
...
module.exports = async function handler(req, res) {
  ...
  try {
      ...
      if (sellerId) {
        ...
        if (searchQ) {
          ...
          /* Unfiltered public listing — most frequently hit, cache for 60s
             FIX: If request comes from an authenticated Admin UI flow, do not drop pending items */
          ...
          /* If it is an admin fetching the main route, show everything so UI doesn't break */
          if (req.headers['x-admin-request'] === 'true' || req.query.isAdmin === 'true') {
            rows = await sql`
              SELECT p.*, u.membership_tier AS seller_tier, u.is_verified AS seller_verified, u.badge_verified AS badge_verified
              FROM products p
              LEFT JOIN users u ON u.id::text = p.seller_id::text
    ...
    if (req.method === 'POST' && !req.query.action) {
      ...
      /* Synchronize public listing indices upon item creation */
      await bustGlobalCache(null, sellerId, productCat, productType);
    ...
    if (req.method === 'PATCH') {
      ...
      /* Purge affected indices if an update succeeds */
    ...
    if (req.method === 'DELETE') {
      ...
      /* Track categories and metadata directly via RETURNING statement prior to row destruction */
      const metaRows = await sql`
        DELETE FROM products WHERE id = ${productId} RETURNING seller_id, cat, type
      `;
      
      /* Completely decouple deleted object records from the caching engines */
      if (metaRows.length) {
        const m = metaRows[0];
    ...
    if (req.query.action === 'approve-edit' && req.method === 'POST') {
      ...
      /* Verify state across global distribution hooks upon change confirmation */
      const productLookup = await sql`SELECT cat, type FROM products WHERE id = ${pid} LIMIT 1`;
    ...
    if (req.query.action === 'promote' && req.method === 'POST') {
      ...
      try {
        ...
        /* Explicitly clear featured items index caches to surface promotion immediately */
        await bustGlobalCache(Number(productId));
