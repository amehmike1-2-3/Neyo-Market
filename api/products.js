...
// ✅ FIX 8: All ID lookups use String matching to prevent BigInt out-of-range crashes
...
/* 🌐 HELPER: Clear all global marketplace caches when data changes */
async function bustGlobalCache(productId, sellerId, cat, type) {
  try {
    const r = _getRedis();
    if (!r) return;
    
    await r.del('products:public');
    await r.del('products:sale');
    
    if (productId) await r.del('products:id:' + String(productId));
    if (sellerId)  await r.del('products:seller:' + String(sellerId));
    if (cat)       await r.del('products:cat:' + String(cat).toLowerCase().trim());
    if (type)      await r.del('products:type:' + String(type).toLowerCase().trim());
    
    console.log('[Redis] Cache layer synchronized successfully.');
  } catch(e) {
    console.warn('[Redis] Cache bust skipped:', e.message);
  }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (id) {
        const cacheKey = 'products:id:' + String(id);
        ...
        const rows = await sql`
          SELECT * FROM products WHERE id = ${String(id)} LIMIT 1
        `;
      ...
      } else if (admin === 'true' || req.query.isAdmin === 'true' || req.headers['x-admin-request'] === 'true') {
    ...
    if (req.method === 'POST' && !req.query.action) {
      ...
      /* Pre-compute all conditionals outside the sql template */
      ...
      await bustGlobalCache(rows[0].id, sellerId, productCat, productType);
    ...
    if (req.method === 'PATCH') {
      ...
      const targetRows = await sql`
        ...
        RETURNING seller_id, cat, type
      `;

      if (targetRows.length) {
        const t = targetRows[0];
        await bustGlobalCache(productId, t.seller_id, t.cat, t.type);
      }
    ...
    if (req.method === 'DELETE') {
      ...
      const metaRows = await sql`
        DELETE FROM products WHERE id = ${String(productId)} RETURNING seller_id, cat, type
      `;
      
      if (metaRows.length) {
        const m = metaRows[0];
        await bustGlobalCache(productId, m.seller_id, m.cat, m.type);
      }
    ...
    if (req.query.action === 'update-product' && req.method === 'POST') {
      ...
      if (!body.id || !sellerId) return jsonErr(res, 400, 'id and sellerId required.');

      /* Ownership check */
      const owns = await sql`
        SELECT id FROM products
        WHERE id = ${String(productId)} AND seller_id = ${sellerId}
        LIMIT 1
      `;
      ...
      await sql`
        ...
        WHERE product_id = ${String(productId)} AND status = 'pending'
      `;
      ...
      await sql`
        ...
        VALUES (${String(productId)}, ${sellerId}, ${JSON.stringify(editData)}, 'pending', NOW())
      `;

      /* Mark the product as having a pending edit */
      await sql`
        UPDATE products SET has_pending_edit = true WHERE id = ${String(productId)}
      `;
    ...
    if (req.query.action === 'pending-edits' && req.method === 'GET') {
      ...
      const edits = await sql`
        ...
        JOIN products p ON p.id::text = pe.product_id::text
      ...
      return res.status(200).json({ ok: true, edits: edits.map(function(e) {
        ...
        return { ...e, edit_data: ed, id: String(e.id), product_id: String(e.product_id) };
      })});
    }
    ...
    if (req.query.action === 'approve-edit' && req.method === 'POST') {
      ...
      const editRows = await sql`
        SELECT * FROM product_edits WHERE id = ${String(editId)} AND status = 'pending' LIMIT 1
      `;
      ...
      await sql`
        ...
        WHERE id = ${String(editId)}
      `;

      const productLookup = await sql`SELECT cat, type FROM products WHERE id = ${String(pid)} LIMIT 1`;
      if (productLookup.length) {
        await bustGlobalCache(pid, edit.seller_id, productLookup[0].cat, productLookup[0].type);
      }
    ...
    if (req.query.action === 'reject-edit' && req.method === 'POST') {
      ...
      await sql`
        ...
        WHERE id = ${String(editId)} AND status = 'pending'
      `;

      const rows2 = await sql`SELECT product_id, seller_id FROM product_edits WHERE id = ${String(editId)} LIMIT 1`;
      if (rows2.length) {
        await sql`UPDATE products SET has_pending_edit = false WHERE id = ${String(rows2[0].product_id)}`;
        try {
          const r = _getRedis();
          if (r) { 
            await r.del('products:id:' + String(rows2[0].product_id)); 
            await r.del('products:seller:' + String(rows2[0].seller_id)); 
          }
        } catch(e) {}
      }
    ...
    if (req.query.action === 'get-views' && req.method === 'GET') {
      ...
      if (!req.query.productId) return jsonErr(res, 400, 'Invalid productId.');
      try {
        const rows = await sql`
          SELECT view_count FROM products WHERE id = ${String(pid)} LIMIT 1
        `;
        if (!rows.length) return jsonErr(res, 404, 'Product not found.');
    ...
    if (req.query.action === 'promote' && req.method === 'POST') {
      ...
      try {
        ...
        await sql`
          ...
          VALUES (${String(productId)}, ${Number(duration || 7)}, ${Number(amount)}, ${String(paystackRef)}, ${expiresAt.toISOString()}, NOW())
        `;

        await bustGlobalCache(productId);
