...
// All ID lookups: String() for all product and user IDs to prevent BIGINT/Large Number crashes
...
function toProduct(r) {
  ...
  return {
    id:             String(r.id), // Ensure client treats product ID safely as a String
    name:           r.name             || '',
    type:           r.type             || 'digital',
    cat:            r.cat              || 'other',
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      /* ── Single product by ID (FIXED: Cast to String for safely checking BIGINT) ── */
      if (id) {
        ...
        // Using user-safe string casting to handle large PostgreSQL BIGINTs safely
        const rows = await sql`
          SELECT * FROM products WHERE id::text = ${String(id)} LIMIT 1
        `;
    ...
    /* ════════════════════════════════════════════════
       PATCH — update product fields (FIXED: id lookup matches string)
    ════════════════════════════════════════════════ */
    if (req.method === 'PATCH') {
      const p = req.body || {};
      if (!p.id) return jsonErr(res, 400, 'Product id is required.');

      const productIdString   = String(p.id);
      ...
      await sql`
        ...
        WHERE id::text = ${productIdString}
      `;
      return res.status(200).json({ ok: true });
    }

    /* ════════════════════════════════════════════════
       DELETE — owner or admin only (FIXED: id string casted)
    ════════════════════════════════════════════════ */
    if (req.method === 'DELETE') {
      ...
      const productIdString = String(rawId);
      ...
      await sql`DELETE FROM products WHERE id::text = ${productIdString}`;
      return res.status(200).json({ ok: true });
    }

    /* ════════════════════════════════════════════════
       POST ?action=update-product (FIXED: uses BIGINT product_id syntax safely)
    ════════════════════════════════════════════════ */
    if (req.query.action === 'update-product' && req.method === 'POST') {
      const body      = req.body || {};
      const productIdString = String(body.id);
      const sellerId  = String(body.sellerId || '');
      if (!body.id || !sellerId) return jsonErr(res, 400, 'id and sellerId required.');

      const owns = await sql`
        SELECT id FROM products
        WHERE id::text = ${productIdString} AND seller_id = ${sellerId}
        LIMIT 1
      `;
      ...
      await sql`
        ...
        WHERE product_id::text = ${productIdString} AND status = 'pending'
      `;
      ...
      await sql`
        ...
        VALUES (${productIdString}::bigint, ${sellerId}, ${JSON.stringify(editData)}, 'pending', NOW())
      `;

      await sql`
        UPDATE products SET has_pending_edit = true WHERE id::text = ${productIdString}
      `;

      try {
        const r = _getRedis();
        if (r) { await r.del('products:seller:' + sellerId); await r.del('products:id:' + productIdString); }
      } catch(e) {}
    ...
    /* ════════════════════════════════════════════════
       POST ?action=approve-edit (FIXED: text match lookup)
    ════════════════════════════════════════════════ */
    if (req.query.action === 'approve-edit' && req.method === 'POST') {
      ...
      const pidString = String(edit.product_id);
      ...
      try {
        const r = _getRedis();
        if (r) { await r.del('products:id:' + pidString); await r.del('products:seller:' + edit.seller_id); }
      } catch(e) {}
    ...
    if (req.query.action === 'reject-edit' && req.method === 'POST') {
      ...
      if (rows2.length) {
        const pidString = String(rows2[0].product_id);
        await sql`UPDATE products SET has_pending_edit = false WHERE id::text = ${pidString}`;
        try {
          const r = _getRedis();
          if (r) { await r.del('products:id:' + pidString); await r.del('products:seller:' + rows2[0].seller_id); }
        } catch(e) {}
      }
    ...
    if (req.query.action === 'track-view' && req.method === 'POST') {
      ...
      try {
        ...
        try {
          const r = _getRedis();
          if (r) await r.del('products:id:' + pidString);
        } catch(e) {}
    ...
    if (req.query.action === 'get-views' && req.method === 'GET') {
      const pidString = String(req.query.productId);
      if (!req.query.productId) return jsonErr(res, 400, 'Invalid productId.');
      try {
        const rows = await sql`
          SELECT view_count FROM products WHERE id::text = ${pidString} LIMIT 1
        `;
        if (!rows.length) return jsonErr(res, 404, 'Product not found.');
    ...
    if (req.query.action === 'promote' && req.method === 'POST') {
      ...
      try {
        ...
        await sql`
          ...
          VALUES (${pidString}::bigint, ${Number(duration || 7)}, ${Number(amount)}, ${String(paystackRef)}, ${expiresAt.toISOString()}, NOW())
        `;
