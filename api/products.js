...
// ✅ FIX 8: Switched schema definitions from INTEGER to BIGINT for millisecond timestamp IDs
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    /* ════════════════════════════════════════════════
       ...
       Uses BIGINT schema constraint safety block
    ════════════════════════════════════════════════ */
    if (req.query.action === 'update-product' && req.method === 'POST') {
      ...
      /* Ensure product_edits table exists with BIGINT type safety constraints built in */
      await sql`
