...
// ─── Payout Email Helper ─────────────────────────────────────────────────────
async function sendPayoutEmails(user, grossAmt, paidOut, fee, manualRef) {
  try {
    ...
    await transporter.sendMail({
      from: 'NeyoMarket <' + adminEmail + '>',
      to:   sellerEmail,
      subject: 'Your Withdrawal of ₦' + grossAmt.toLocaleString() + ' Has Been Approved',
      html: '<div style="font-family:sans-serif;max-width:480px;margin:auto;padding:24px;border:1px solid #eee;border-radius:10px">'
        + '<h2 style="color:#c9922a">NeyoMarket Payout Approved ✅</h2>'
        ...
        + '<p style="color:#888;font-size:.85rem">If you have any questions, contact us via WhatsApp or reply to this email.</p>'
        + '<p style="color:#c9922a;font-weight:700">NeyoMarket Team</p>'
        + '</div>'
    });

    // Email to ADMIN (action reminder)
    await transporter.sendMail({
      from: 'NeyoMarket System <' + adminEmail + '>',
      to:   adminEmail,
      subject: '⚠️ ACTION REQUIRED: Send ₦' + paidOut.toLocaleString() + ' to ' + sellerName,
      html: '<div style="font-family:sans-serif;max-width:480px;margin:auto;padding:24px;border:2px solid #c9922a;border-radius:10px">'
        + '<h2 style="color:#c9922a">⚠️ Manual Payout Required</h2>'
        + '<p>A withdrawal has been approved. Please send the funds manually:</p>'
        + '<table style="width:100%;border-collapse:collapse;margin:16px 0">'
        ...
        + '<tr><td style="padding:8px;border:1px solid #eee;color:#888">Reference</td><td style="padding:8px;border:1px solid #eee">' + manualRef + '</td></tr>'
        + '</table>'
        + '<p style="color:#e53e3e;font-weight:700">Please send this payment within 24 hours.</p>'
        + '</div>'
    });

    console.log('[paystack] Payout emails sent for ref ' + manualRef);
  } catch (emailErr) {
    console.warn('[paystack] Payout email failed (non-fatal):', emailErr.message);
  }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'approve-payout') {
      ...
      const { withdrawalId, userId, amount, flatFee, netAmount } = req.body || {};
      ...
      if (grossAmt > bal)
        return jsonErr(res, 400, 'Seller balance (N' + bal.toLocaleString() + ') is less than requested (N' + grossAmt.toLocaleString() + ').');
      ...
      return res.status(200).json({
        ...
        fee,
        netPaid:   paidOut,
        message:   'Withdrawal approved. Please send N' + paidOut.toLocaleString() + ' manually to ' + (user.payout_aname || user.name) + ' — ' + (user.payout_acct || 'no account saved') + ' (' + (user.payout_bank || '') + ')',
      });
    }
