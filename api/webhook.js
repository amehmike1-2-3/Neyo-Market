  ...
  try {
    ...
    if (!orders.length) {
      ...
      try {
        ...
        const isPhysical  = customerAddr && customerAddr !== 'Digital Delivery';
        const orderMode   = isPhysical ? 'physical' : 'digital';
        ...
        /* Deterministic delivery code — matches generateDVC() in frontend */
        let dvcHash = 0;
        const dvcStr = String(orderId);
        for (let i = 0; i < dvcStr.length; i++) {
          dvcHash = ((dvcHash << 5) - dvcHash) + dvcStr.charCodeAt(i);
          dvcHash |= 0;
        }
        const deliveryCode = String(Math.abs(dvcHash) % 900000 + 100000);

        /* Basic commission split — free tier defaults */
        const TIER_RATES = { digital: 0.10, physical: 0.05 };
        const platformRate = isPhysical ? TIER_RATES.physical : TIER_RATES.digital;
        ...
        console.log('[webhook.js] ✅ Order CREATED from webhook —', orderId, '₦' + paidAmount, 'status:', orderStatus);
